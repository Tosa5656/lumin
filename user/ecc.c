...
static char *lbl_new(void) {
    int n = lbl_cnt++;
    int i = 63;
    lbl_buf[i] = '\0';
    do { lbl_buf[--i] = '0' + (n % 10); n /= 10; } while (n);
    lbl_buf[--i] = 'L'; lbl_buf[--i] = '.';
    return lbl_buf + i;
}
...
static Value parse_primary(void) {
    ...
    if (lex.tk.t == TK_NUM) {
        ...
        char lbl[64];
        {
            int n = sc, len = 0, tmp = n;
            do { len++; tmp /= 10; } while (tmp);
            lbl[0] = '.'; lbl[1] = 's'; lbl[2] = 't'; lbl[3] = 'r';
            int p = 4 + len;
            lbl[p] = '\0';
            do { lbl[--p] = '0' + (n % 10); n /= 10; } while (n);
        }
        int saved_sec = cur_sec; cur_sec = sec_rodata;
        label_def(lbl);
        sec_emit(sec_rodata, str, len);
        cur_sec = saved_sec;
        emit_mov_ri(0, 0);
        fixup_add(8, 1, lbl);
...
/* ─── type parser ─── */
static Ty *ptr_to(Ty *b) { Ty *p = calloc(1, sizeof(Ty)); p->k = TY_PTR; p->sz = 8; p->to = b; return p; }
static Ty *parse_type(void) {
    int uns = 0;
